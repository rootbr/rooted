#!/usr/bin/env python3
"""Grade a craft review run of the fixture against evals/expected.json — mechanically, on
(rule_id, file, symbol) triples; no judge model.

Usage: python3 evals/grade.py --expected evals/expected.json --verdicts craft/verdicts.json [--findings craft/findings.json]
...
carries its rule_id and file and its symbol's routine part; a control passes when no
...
def routine_part(symbol):
    """The routine or type name after the last separator a language uses (`#`, `.`, `::`, `:`,
    a space after `func`/`fn`/`def`), with `()` stripped, lower-cased."""
    s = str(symbol or "").strip()
    for prefix in ("func ", "fn ", "def ", "file:"):
...
def matches(f, file, symbol):
    """`symbol` is one anchor or a list of the anchors a card admits; the finding matches when
    its routine part equals any of them."""
    symbols = symbol if isinstance(symbol, list) else [symbol]
    return f.get("file") == file and any(routine_part(f.get("symbol")) == routine_part(x) for x in symbols)
...
def main(argv):
    ...
    if not opts["verdicts"]:
        sys.exit(__doc__)
    with open(opts["expected"], encoding="utf-8") as fh:
        exp = json.load(fh)
    with open(opts["verdicts"], encoding="utf-8") as fh:
        data = json.load(fh)
    ...
    for s in exp["seeds"]:
        ...
        sym = s["symbol"] if isinstance(s["symbol"], str) else " | ".join(s["symbol"])
        print(f"  {'PASS' if ok else 'FAIL'}  {s['id']}  ({s['rule_id']} {sym}){extra}"
