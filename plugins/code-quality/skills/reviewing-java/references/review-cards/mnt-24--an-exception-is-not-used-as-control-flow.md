---
title: An exception is not thrown to be caught by the same method as a branch, and an expected outcome is tested rather than caught
rule_id: MNT-24
...
# An exception is not thrown to be caught by the same method as a branch, and an expected outcome is tested rather than caught

## Thesis
A `throw` inside a `try` whose own `catch`, in the same method, handles it is a `goto` and is not written; a condition the code can test — a value that fails a check, a key that may be absent, an index that may be out of range, a reference that may be `null` — is tested with a plain check, an early `return` or an `Optional`, and not provoked into an exception that a `catch` in the same method turns into a branch.
