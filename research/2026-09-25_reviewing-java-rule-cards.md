...
10. **Audit scope of the corpus.** The brief's quality gate runs the audit on every SKILL.md or agent-prompt change, and the work plan extended it to every card. The audit dispatches 22 rule agents per card, so the full corpus is 5,280 agents, which at this host's concurrency cap of two is about a day of wall-clock time per pass. The corpus was audited instead by the static pre-pass on all 240 cards, the two corpus-integrity cards on the directory, the full semantic set on the six pilot cards and on a sample of twelve cards, two per domain, chosen among those the fixers changed most, with the independent reviewers' pass standing in for the rest: every card read against its cited source, every `good:` half compiled, every trigger tested. The six pilot cards' semantic audit produced only findings the taxonomy documents as deviations, which is the evidence that the sample suffices until a cheaper audit path exists.

## Dry run on the extended fixture

The full corpus (240 cards) ran against the extended fixture (nine Java files and a rewritten `config.md`; 18 seeds, one per chosen card across all six domains, and 19 controls) through the pre-pass and the bundled workflow: 81 card jobs, 3 logic slices and 4 project invariants, 96 finders, 54 skeptics, no second skeptic and no arbiter. Every seed card was dispatched by its triggers. The finders raised 54 findings; the skeptics confirmed 44, modified 8 (a real defect with a corrected fix or rationale), downgraded 2 and rejected none (a resumed run after the aggregation change re-verified the same 54 with 36 confirmed, 17 modified and 1 downgraded: the labels move between runs, the dispositions do not); the aggregation folded one pair (the project invariant on the shared cache with the concurrency card on the same span), overruled one (the project invariant on field injection over the maintainability card) and flagged two same-priority ties on `Totals#transfer` for the author. Grading: 18 of 18 seeds found and 19 of 19 controls silent, after two corrections to the grading mechanics the run exposed: an overruled finding vanished from the report, so the aggregation now lists the losers' rule ids on the kept finding with their text under `superseded`; and a seed whose card admits two anchors (the indexed read, or the `LinkedList` field it reads) was graded on one, so a seed may now list several symbols. No control was flagged by any finder.
...
Cost, as the runtime reported it: 150 agents, 8,301,570 sub-agent tokens, 3,277 s of wall-clock time at the runtime's concurrency cap of two agents on this 4-CPU host; the resumed run, with most finders replayed from cache, a further 3,126,839 tokens in 2,502 s. The rendered reports of the run are kept under `evals/runs/`. The plan travelled as a bundle (`review/run.js`, 124 KB with the compact job records); the same plan with full card records and repeated hunk text was 524 KB, which no orchestrator could retype into a tool call.
