...
| `## Example` form (C-F3) | a fenced Java `bad:` / `good:` pair of up to about eight lines, with generic names (`cache`, `load`, `k`) | a three-line everyday-domain pair cannot carry a Java defect; the reader is a Java reviewer and the card's domain is its own. A documented deviation from C-F3 |
...
- `## Example` — one fenced `java` block holding a `bad:` line group and a `good:` line group, up to about eight lines in total, with generic names (`cache`, `load`, `k`). The `good:` half compiles in a plausible enclosing class.
