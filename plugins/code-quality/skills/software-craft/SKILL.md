---
name: software-craft
description: Build code by the craft cards and review it against them, in any mainstream language (Java, Python, TypeScript, Go, Rust first-class). Trigger on "build", "implement", "extend", "refactor", "add tests", "scaffold a module" ("implement rate limiting in src/api/limits.py with tests") — the developer agent writes the code and a card-dispatched craft review gates it — and on a standalone craft review of a diff in any language ("review this change for craft", "check PR #42 against the craft rules"). Any git repository, zero config (optional .claude/software-craft/config.md tunes invariants, modules and report paths); runs read-only git diff and gh pr view, a deterministic pre-pass matching rule cards to the diff, one Workflow with a finder per triggered card and a skeptic per finding, one fix round, two reports under craft/. NOT for a Java-specific review against Java invariants, which another skill covers; NOT for debugging as the primary task; NOT for the reserved specialties (low-latency or HFT tuning, concurrency correctness, security-critical design, ML or algorithm research); NOT for searching the codebase.
