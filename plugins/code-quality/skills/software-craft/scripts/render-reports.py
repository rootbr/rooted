#!/usr/bin/env python3
"""Render the two craft review reports from the workflow's verified findings.

Inputs:
  --verdicts craft/verdicts.json    the workflow's `findings` array after Verify: each
                                    finding carries `verdict` and, for a rejected Major,
                                    a `ruling`
  --plan     craft/plan.json        the inventory (diff, files, languages, size) and the
                                    card index, whose paths supply each finding's one-line source
  --out      <path>                 the review report; the rejection report lands next to
                                    it with a `-rejections` suffix
  --title    <text>                 the report title (task, ticket id or branch)
  --passes   <text>                 one line naming the passes that ran (optional)
  --built    <file>                 optional: the developer agent's report, quoted in a
                                    "What was built" section
  --fix-round <text>                optional: one line on the fix round (findings fixed, left)
...
a downgraded finding also stays in the review report at its new severity.
"""
import json
import os
import re
...
SEV_ORDER = {"major": 0, "minor": 1, "suggestion": 2}
...
def lang_of(plan, path):
    for f in plan.get("inventory", {}).get("files", []):
        if f.get("path") == path:
            return f.get("language") or ""
    return ""


def render(findings, plan, title, passes, built, fix_round):
    ...
    grave = [f for f in shown if final_severity(f) == "major"]
    light = [f for f in shown if final_severity(f) in ("minor", "suggestion")]
    rec = "Fix before merge" if counts["major"] else ("Merge with fixes" if counts["minor"] else "Merge")
    languages = ", ".join(f"{k} {v}" for k, v in sorted((inv.get("languages") or {}).items())) or "n/a"

    out = [f"# {title} Craft Review", "", "## Executive Summary",
           f"- Reviewed: `{inv.get('diff_ref', '?')}` ({inv.get('mode', '?')}) — {inv.get('file_count', 0)} files ({inv.get('size_class') or 'n/a'}); languages: {languages}",
           f"- Passes: {passes or default_passes(plan)}",
           f"- Findings: {len(shown)} ({counts['major']} major, {counts['minor']} minor, {counts['suggestion']} suggestion)",
           ...
           f"- Fix round: {fix_round or 'none'}",
           f"- Recommendation: {rec}", ""]
    if built:
        out += ["## What was built", "", built.strip(), ""]
    ...
    for f in grave:
        ...
        out += [f"### {f.get('id', f['rule_id'])}: {title_of(f, cards, plan)}",
                ...
                "- **Code**:", f"  ```{lang}", indent(b["code"]), "  ```",
                f"- **Problem**: {b['problem']}",
                "- **Suggested fix**:", f"  ```{lang}", indent(b["fix"]), "  ```",
...
def main(argv):
    opts = {"verdicts": "craft/verdicts.json", "plan": "craft/plan.json", "out": None, "title": "Review", "passes": "",
            "built": None, "fix_round": ""}
    i = 0
    while i < len(argv):
    ...
    built = ""
    if opts["built"]:
        with open(opts["built"], encoding="utf-8") as handle:
            built = handle.read()
    report, rejections, summary = render(findings, plan, opts["title"], opts["passes"], built, opts["fix_round"])
