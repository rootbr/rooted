...
G-14. Finders search files the plan never matched, or a card's trigger list looks truncated in the run? The plan was retyped into the tool call by hand. Bundle it instead: `scripts/bundle-run.py` writes `review/run.js` with the plan embedded verbatim from `review/plan.json`, and the Workflow call names that script with no `args`; `root` in the bundle is the absolute repository path.
