...
Call the tool with `scriptPath` aimed straight at this shipped script and `args` given as a JSON object, never as a JSON-encoded string. An inline wrapper that re-invokes the script through the in-script `workflow()` hook forfeits a shipped defense: the script parses string `args` itself, while a wrapper reads its own `args` raw, so a stringified call resolves every field to `undefined` and dies before its first agent with `TypeError: workflow() expects a workflow name (string) or {scriptPath: string}` (G-28).
