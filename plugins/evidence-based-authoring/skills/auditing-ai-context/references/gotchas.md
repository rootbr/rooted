...
G-28. The run dies in milliseconds with `TypeError: workflow() expects a workflow name (string) or {scriptPath: string}` — zero agents, empty logs? The main agent authored an inline wrapper workflow that re-invokes the shipped script via the in-script `workflow()` hook, and passed the Workflow tool's `args` as a JSON-encoded string. The wrapper reads its `args` raw, so every field resolves to `undefined`, and the inner `workflow({scriptPath: undefined}, …)` throws before the first agent (verified hands-on 2026-08-08: the persisted run record stores `args` as one JSON string and the wrapper crashed at its first field access). The shipped `scripts/audit-workflow.js` parses string `args` itself, so the same stringified call survives a direct invocation — a wrapper stands in front of that defense while adding no capability the tool call lacks. Fix: re-invoke the Workflow tool with `scriptPath` pointing straight at the shipped script and `args` as a JSON object — the Phase 1 invocation verbatim.
