#!/usr/bin/env python3
"""Bundle one review run: write review/run.js = scripts/review-workflow.js with the run's
arguments embedded as `const EMBEDDED_ARGS = {...}` right after the `meta` block.
...
the main agent calls `Workflow({ scriptPath: "<repo>/review/run.js" })` with no args.

Usage:
  bundle-run.py --root <repo> --plan review/plan.json --intent <file|-> --context <file|->
                [--stage all|find|verify] [--findings review/findings.json]
                [--tiers <json-file>] [--agent-types <json-file>] [--out review/run.js]
...
def bundle(script, args_obj):
    ...
    if start is None:
        sys.exit("bundle-run: review-workflow.js has no `export const meta = {` line")
    ...
    return "".join(lines[: end + 1]) + "// Embedded by scripts/bundle-run.py — the plan verbatim from review/plan.json.\n" + embedded + "".join(lines[end + 1:])


def main(argv):
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--root", required=True, help="absolute path of the repository under review")
    ap.add_argument("--plan", default="review/plan.json")
    ...
    ap.add_argument("--findings", default=None, help="review/findings.json (stage verify)")
    ...
    ap.add_argument("--script", default=os.path.join(HERE, "review-workflow.js"))
    ap.add_argument("--out", default="review/run.js")
    ...
    for key in ("inventory", "cards", "jobs", "slices"):
        if key not in plan:
            sys.exit(f"bundle-run: plan lacks \"{key}\" (the plan is written by scripts/static-review.py)")
    args_obj = {
        "root": root,
        "stage": opts.stage,
        "plan": plan,
